...
export default function SiteHeader({ auth }: { auth: Auth }) {
    ...
    return (
        <>
            <header className="fixed inset-x-0 top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur-xl">
                <div className="mx-auto flex h-[74px] w-full max-w-[1360px] items-center justify-between px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="hidden items-center gap-5 sm:flex">
                        {!isAuthenticated && <Link href={login()} className="min-h-11 rounded-lg px-1.5 py-3 text-sm font-medium text-slate-700 transition hover:text-brand-600 focus:outline-none focus-visible:ring-4 focus-visible:ring-brand-600/20">Login</Link>}
                        <Link href={isAuthenticated ? dashboard() : register()} className="inline-flex min-h-11 items-center gap-2 rounded-[10px] bg-brand-600 px-5 py-3 text-sm font-semibold text-white shadow-[0_8px_24px_rgba(15,23,42,0.08)] transition hover:bg-brand-700 focus:outline-none focus-visible:ring-4 focus-visible:ring-brand-600/25 active:bg-brand-800">
                    ...
                    <>
                        ...
                        <aside id="mobile-navigation" aria-label="Mobile navigation" className="site-mobile-panel fixed right-0 top-0 z-[70] flex h-[100dvh] w-[min(88vw,360px)] flex-col border-l border-slate-200 bg-white px-5 pb-7 pt-5 shadow-[-12px_0_32px_rgba(15,23,42,0.1)] sm:hidden">
                            ...
                            <div className="mt-auto grid gap-3 border-t border-slate-200 pt-6">
                                {!isAuthenticated && <Link href={login()} onClick={closeMenu} className="min-h-12 rounded-[10px] px-4 py-3 text-center text-sm font-semibold text-slate-800 hover:bg-slate-50">Login</Link>}
                                <Link href={isAuthenticated ? dashboard() : register()} onClick={closeMenu} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-[10px] bg-brand-600 px-4 py-3 text-sm font-semibold text-white hover:bg-brand-700">{isAuthenticated ? 'Go to Dashboard' : 'Sign Up Free'} <ArrowUpRight size={15} /></Link>
                            </div>
                        </aside>
                    </>
                )}
            ...
            {showBackToTop && <button type="button" onClick={scrollToTop} aria-label="Back to top" title="Back to top" className="fixed bottom-6 right-6 z-50 flex h-12 w-12 items-center justify-center rounded-full bg-brand-600 text-white shadow-[0_12px_28px_rgba(37,99,235,0.35)] transition duration-200 hover:-translate-y-1 hover:bg-brand-700 focus:outline-none focus-visible:ring-4 focus-visible:ring-brand-600/30"><ArrowUp size={20} strokeWidth={2.5} /></button>}
        </>
    );
}
