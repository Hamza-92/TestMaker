        ...
        $classSubjects = ClassSubject::join('subjects', 'subjects.id', '=', 'class_subjects.subject_id')
            ...
            ->whereExists(function ($query): void {
                $query->selectRaw('1')
                    ...
                    ->whereColumn('chapters.pattern_id', 'class_subjects.pattern_id')
                    ->whereColumn('chapters.class_id', 'class_subjects.class_id')
                    ->whereColumn('chapters.subject_id', 'class_subjects.subject_id')
                    ->where('chapters.status', 1)
                    ->where('questions.status', 1)
                    ->where('question_types.status', 1)
                    ->whereIn('questions.source', Question::sourceValues());
            })
